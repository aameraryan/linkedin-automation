...
csv_file = open('itm-it_&_services-51-200.csv', 'a')
...
for page in range(1, 46):
    ...
    url = 'https://www.linkedin.com/sales/search/people?companySize=D&geo=us%3A0&industry=96&logHistory=false&logId=1802049545' \
          '&page={page}&searchSessionId=3kxDWHa6Qt%2BDQF0JVAOMdw%3D%3D&title=IT%2520Manager%3A65&titleTimeScope=CURRENT'.format(page=str(page))
    driver.get(url)
    ...
    for article in articles:
        ...
        try:
            my_dict = {'name': name, 'job_title': job_title, 'company': company,
                       'location': location, 'exp': exp, 'detail': detail, 'misc': misc, 'link': link}
            time.sleep(0.03)
